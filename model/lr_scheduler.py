...
class CosineAnnealingLR_Restart(_LRScheduler):
    def __init__(self, optimizer, T_period, restarts=None, weights=None, eta_min=0, last_epoch=-1, use_two_lr=False):
        self.T_period = T_period
        self.T_max = self.T_period[0]  # current T period
        if use_two_lr:
            self.eta_min = [eta_min,eta_min/10]
        else:
            self.eta_min = eta_min
        self.use_two_lr = use_two_lr
        self.restarts = restarts if restarts else [0]
        self.restarts = [v + 1 for v in self.restarts]
        self.restart_weights = weights if weights else [1]
        self.last_restart = 0
    ...
    def get_lr(self):
        if self.last_epoch == 0:
            ...
            if self.use_two_lr:
                ...
                for base_lr, group, eta_min in zip(self.base_lrs, self.optimizer.param_groups, self.eta_min)
                ]
            else:
        ...
        if self.use_two_lr:
            return [(1 + math.cos(math.pi * (self.last_epoch - self.last_restart) / self.T_max)) /
                ...
                for group, eta_min in zip(self.optimizer.param_groups, self.eta_min)]
        else:
            return [(1 + math.cos(math.pi * (self.last_epoch - self.last_restart) / self.T_max)) /
                    (1 + math.cos(math.pi * ((self.last_epoch - self.last_restart) - 1) / self.T_max)) *

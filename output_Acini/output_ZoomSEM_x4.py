...
def main():
    ...
    im_path_SR = os.path.join(root_path, 'output_data', 'ZoomSEM_Acini_x4')
    assert not os.path.exists(im_path_SR)
    scale = 4
    pretrain_path = os.path.join(root_path, 'pretrained', 'ZoomSEM_FT_x4.pth')
    utils.mkdirs([im_path_SR])
    opt_path = os.path.join(root_path, 'option', 'train_ZoomSEM_x4_Acini.yml')
